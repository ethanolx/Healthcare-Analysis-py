...
ax1: axes.SubplotBase = plt.subplot2grid((1, 5), (0, 0), colspan=3)
ax2: axes.SubplotBase = plt.subplot2grid((1, 5), (0, 3), colspan=2)
...
# Preparation
years: np.ndarray = np.unique(defective_vision_rates['year'])
...
# Add annotations
ax2.text(1.12, np.median(male_data) + 0.1, f'Median (M): {np.median(male_data)}%', font={'size': 7})
