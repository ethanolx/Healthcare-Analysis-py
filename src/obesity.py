...
_, bins, _ = ax3.hist(obese_percent, bins=10, density=True, edgecolor='k')  # type: ignore
...
ax3.annotate('13% of all students are obese on average', xy=(12.9, 0.152), xytext=(14, 0.25), arrowprops={'facecolor': 'tab:orange', 'arrowstyle': 'fancy'}, font={'size': 9, 'family': 'Ubuntu Mono', 'weight': 'bold'}, color='tab:orange', ha='left')  # type: ignore

mu, sigma = np.mean(obese_percent), np.std(obese_percent)
ax3.plot(bins, 1 / (sigma * np.sqrt(2 * np.pi)) * np.exp(- (bins - mu) ** 2 / (2 * sigma**2)), linewidth=2)  # type: ignore
ax3.set_xlabel('Percentage of Students Screened Who are Obese / %') #type: ignore
ax3.set_ylabel('Probability') #type: ignore
ax3.set_title('Distribution of Obesity Rates in SG') #type: ignore
